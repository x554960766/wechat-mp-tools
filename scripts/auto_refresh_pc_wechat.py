...
def _find_dropdown_filehelper_box(boxes: list) -> any:
    ...
    # 3. 收集所有位于聊天记录上方、且排除顶部搜索框输入文字（top > 45）的「文件传输助手」候选块
    cands = []
    for b in boxes:
        if b.top >= chat_top or b.top < 45:
            continue
        txt = b.text.strip()
        # 匹配包含“文件传输助手”的文本块
        if "文件传输助手" in txt:
    ...
    try:
        ...
        def _is_in_filehelper_window(w) -> bool:
            ...
            if img is None:
                return False
            boxes = ocr(img)
            r_start = int(img.shape[1] * 0.35)
            return any("文件传输助手" in b.text and b.top < 80 and b.left > r_start for b in boxes)
        ...
        if not _is_in_filehelper_window(main_win):
            for attempt in range(1, 3):
                ...
                left_limit = int(img_cur.shape[1] * 0.38)

                # 优先检查左侧会话列表中是否直接可见「文件传输助手」（排除搜索框 b.top > 50）
                list_helper = next((b for b in boxes_cur if b.left < left_limit and b.top > 50 and "文件传输助手" in b.text), None)
                if list_helper:
                    ...
                    search_box = next((b for b in boxes_cur if b.left < left_limit and b.top < 80 and any(k in b.text for k in ("搜索", "Search"))), None)
                    if search_box:
                    ...
                    drop_cands = [b for b in boxes2 if b.left < left_limit + 100]
                    target_box = _find_dropdown_filehelper_box(drop_cands)
                    if target_box:
                        sx, sy = cs_cur.img_to_screen(target_box.center[0], target_box.center[1])
                        logger.info("✅ 在主窗口浮层中精确定位到「功能 -> 文件传输助手」[%s]，点击坐标 (%d, %d)", target_box.text, sx, sy)
                        move_and_click(sx, sy)
                        human_sleep(0.8, 1.2)
                    else:
                        logger.info("👉 未在浮层 OCR 中精确定位到候选框，尝试键盘向下选中并回车...")
                        press("down")
                        human_sleep(0.2, 0.3)
                        press("return")
                        human_sleep(0.5, 0.8)
                        fallback_sx = main_win.x + 150
                        fallback_sy = main_win.y + 75
                        move_and_click(fallback_sx, fallback_sy)
                        human_sleep(0.8, 1.2)
