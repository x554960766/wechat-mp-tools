...
> [!TIP]
> **无需配置本地开发环境，开箱即用！**
> 本项目已通过 GitHub Actions 自动构建出 Windows 与 macOS 的可执行客户端。请直接前往 [👉 GitHub Releases 1.4.0 下载页面](https://github.com/x554960766/wechat-mp-tools/releases/tag/v1.4.0) 下载最新版本。
...
- **一键采集全部关注（1.4.0 新增）**：在视频号页面一键枚举并**增量采集**所有已关注博主的作品清单。内置多重低风控机制——令牌桶限流、增量「命中已采即停」、连续失败熔断退避、对数正态节流抖动，并支持单作者条数上限与调用埋点（便于自助评估风控概率）。
