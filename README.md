...
> [!TIP]
> **无需配置本地开发环境，开箱即用！**
> 本项目已通过 GitHub Actions 自动构建出 Windows 与 macOS 的可执行客户端。请直接前往 [👉 GitHub Releases 1.7.0 下载页面](https://github.com/x554960766/wechat-mp-tools/releases/tag/v1.7.0) 下载最新版本。

---

## 功能概览

### 📱 微信公众号（1.7.0 重大更新：全量升级至微信读书模式）
- **微信读书架构重构**：全面替换已受微信防刷风控封禁的旧版创作者后台，全量对接微信读书平台协议（参考 `wewe-rss`）。
- **无弹窗二维码登录**：点击“添加账号”后前端页面卡片直接渲染二微码图片，无需弹出外部浏览器窗口，手机微信扫一扫即可秒级完成登录。
- **微信读书账号池与 429 自愈**：支持添加多个微信读书采集账号自动轮换。遇到 429 频繁限制时自动切换冷却并调取池内下一个可用账号，保障采集任务持续稳定。
- **文章链接一键解析添加**：支持粘贴公众号任意一篇文章链接（`https://mp.weixin.qq.com/s/...`）极速反查解析并添加/升级公众号。系统自动识别并清理旧版不兼容记录。
- **文章离线下载与 RSS 订阅**：离线保存文章正文（支持 HTML、图片、音频和视频等），支持增量拉取与标准 RSS 2.0 输出。
- **画廊与特殊排版**：支持画廊/贴图类文章排版解析。
- **异常容错与抗风控**：内置拟人随机抖动延迟 (`random.uniform`) 与 `curl_cffi` (Chrome TLS 指纹) 请求伪装，彻底规避 SSL 握手断开与风控拦截。
